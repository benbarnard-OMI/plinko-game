<script lang="ts">
  ...
  // Add local state for hiding/unhiding controls
  ...
  <div class="flex w-full flex-col gap-4 bg-gray-600 p-5 lg:w-80">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2">
        ...
        <button
          onclick={() => controlsHidden = true}
          class="rounded p-2 text-white transition hover:bg-gray-500"
          aria-label="Hide Controls"
        >
          <EyeSlash class="size-5" weight="bold" />
        </button>
      </div>
    </div>
    ...
    <div class="space-y-2">
      ...
      <select
        ...
        class="w-full rounded border border-gray-500 bg-gray-700 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
      >
...
{#if controlsHidden}
  <div class="flex items-center justify-between">
    ...
    <div class="flex gap-2">
      <button
        onclick={() => controlsHidden = false}
        class="rounded p-2 text-white transition hover:bg-gray-500"
        aria-label="Unhide Controls"
      >
        <Eye class="size-5" weight="bold" />
      </button>
    </div>
  </div>
{/if}
